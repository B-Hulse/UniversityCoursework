...
// This is defined so that the static callbacks know what instance of atten_trigrid_window to pass the input to
atten_trigrid_window* ACTIVEWIN;

// Constructor to initialise variables
atten_trigrid_window::atten_trigrid_window() {
    mDown = false;
    mXPos = 0.; mYPos = 0.;
    sVert = -1;
    scale = 0.6;

    // Set the active window pointer to point to this instance of atten_trigrid_window
    // This allows the instance to receive inputs
    ACTIVEWIN = this;
}

void atten_trigrid_window::initGridVerts() {
...
void atten_trigrid_window::initGridFaces() {
...
void atten_trigrid_window::show() {
...
void atten_trigrid_window::mbUpdate(int button, int action) {
...
void atten_trigrid_window::mpUpdate(double xPos, double yPos) {
...
void atten_trigrid_window::fillGridFaces() {
...
void atten_trigrid_window::invBaryInterp(Vert2f p, Vert2f a, Vert2f b, Vert2f c, float* alpha, float* beta) {
...
void atten_trigrid_window::baryInterp(Vert2f* p, Vert2f a, Vert2f b, Vert2f c, float alpha, float beta) {
...
void atten_trigrid_window::loadMesh(fi2d_mesh inMesh) {
    this->mesh = inMesh;
}

// Returns the edited mesh
fi2d_mesh atten_trigrid_window::saveMesh() {
    return this->mesh;
}

// Uses old school opengl to draw the mesh to screen
void atten_trigrid_window::drawMesh() {
    glColor4f(1., 0., 0., 1.);
    glLineWidth(2);
    glPushMatrix();
...
void atten_trigrid_window::drawGrid() {
    glColor4f(0., 0., 0., 1.);
    glPushMatrix();
...
void atten_trigrid_window::changeGrid(float x, float y) {
    float k = 1;
    // The change in pos for the grabbed vert
    Vert2f movement{ x - gridVerts[sVert].x,y - gridVerts[sVert].y };
    ...
    {
        if (i != sVert) {
            Vert2f dPos = movement;

            float currMulti = pow(0.2 / (getDistBetweenVerts(gridVerts[i], gridVerts[sVert])), k);
            if (currMulti > 1) {
                currMulti = 1;
            }
            ...
            gridVerts[i].x += dPos.x;
            gridVerts[i].y += dPos.y;
        }
    }
...
void atten_trigrid_window::changeGridSec(size_t fIndex) {
...
//   / |
// c1--c2
bool atten_trigrid_window::vertInside(Vert2f p, Vert2f c1, Vert2f c2, Vert2f c3) {
    bool aboveA = false;
    bool aboveB = false;
    bool aboveC = false;
...
float atten_trigrid_window::halfPlane(Vert2f p, Vert2f a, Vert2f b) {
...
void atten_trigrid_window::getMousePos(float* x, float* y) {
...
float atten_trigrid_window::getDistBetweenVerts(Vert2f a, Vert2f b) {
    float dX = a.x - b.x;
    float dY = a.y - b.y;
...
float atten_trigrid_window::isCCW(Vert2f a, Vert2f b, Vert2f c) {
...
void atten_trigrid_window::mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ACTIVEWIN->mbUpdate(button, action);
}

void atten_trigrid_window::cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
