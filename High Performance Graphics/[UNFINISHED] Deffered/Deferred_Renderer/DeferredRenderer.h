...
public:
    ...
    // A light source struct to pass to the shader
    struct LightSource {
    ...
    struct {
        LightSource lights[1]; // Up to 6 lights
        glm::vec4 viewPos;
...
private:
    ...
    struct FrameBuffer {
        ...
        FBAttachment col, normal, spec, depth;
        VkRenderPass renderPass;
    } geoFrameBuffer;

    VkCommandBuffer geoCMDBuffer;
    VkSemaphore geoSemaphore;
    VkPipeline geoPipeline;

    VkDescriptorSet geoDescriptorSet;

    VkSampler colAttachmentSampler;
    ...
    // UBO Buffers and memory
    VkBuffer geoVertUBOBuffer;
    VkDeviceMemory geoVertUBOBufferMemory;
    VkBuffer  finalFragUBOBuffer;
    VkDeviceMemory finalFragUBOBufferMemory;
    ...
    void createGeoFramebuffers();
    void createGeoAttachments();
    void createGeoCMDBuffer();
    void createAttachment(VkFormat format, VkImageUsageFlagBits usage, FBAttachment* attachment);
    ...
    void ASSERT_VK_SUCCESS(VkResult res);
};
