...
#include "glm_stream_overloads.h"
...
{ // constructor
	...
	mvMatrixStack.push(glm::mat4(0.0f));
	pMatrixStack.push(glm::mat4(0.0f));
} // constructor
...
{ // PopMatrix()
	...
	if (currMatrixStack->size() == 0) {
		currMatrixStack->push(glm::mat4(0.0f));
	}
} // PopMatrix()
...
{ // LoadIdentity()
	currMatrixStack->top() = glm::mat4(1.0f);
} // LoadIdentity()
...
{ // MultMatrixf()
	...
	// Convert the matrix passed in to a glm::mat4 instance
	glm::mat4 inMat;
...
{ // Rotatef()
	...
	glm::mat4 rotMat = glm::rotate(glm::mat4(), angle, glm::vec3(axisX, axisY, axisZ));
...
{ // Scalef()
	glm::mat4 scaleMat = glm::scale(glm::mat4(), glm::vec3(xScale, yScale, zScale));
...
{ // Translatef()
	glm::mat4 transMat = glm::translate(glm::mat4(), glm::vec3(xTranslate, yTranslate, zTranslate));
...
{ // Color3f()
	...
	drawColour.r = red * 255.f;
	drawColour.g = green * 255.f;
	drawColour.b = blue * 255.f;
	drawColour.a = 255.f;
} // Color3f()
...
{ // Vertex3f()
	...
	//newVert.position = glm::vec4(x, y, z);
	newVert.position = glm::vec4(x, y, z, 1.0f);
	newVert.colour.r = drawColour.r;
	newVert.colour.g = drawColour.g;
	newVert.colour.b = drawColour.b;
	newVert.colour.a = drawColour.a;
...
{ // Light()
	if (parameterName & FAKEGL_POSITION) {
		lightPosition = glm::vec4(parameterValues[0],
			parameterValues[1],
			parameterValues[2],
			parameterValues[3]);
...
{ // Clear()        
	...
	if (mask & FAKEGL_COLOR_BUFFER_BIT) {
		for (size_t i = 0; i < frameBuffer.height * frameBuffer.width; i++)
		{
			...
			frameBuffer.block[i].r = fbClearColor.r;
			frameBuffer.block[i].g = fbClearColor.g;
			frameBuffer.block[i].b = fbClearColor.b;
			frameBuffer.block[i].a = fbClearColor.a;
		}
	}
...
{ // ClearColor()
	// Scale up the floats from 0-1 -> 0->255
	fbClearColor.r = red * 255.f;
	fbClearColor.g = green * 255.f;
	fbClearColor.b = blue * 255.f;
	fbClearColor.a = alpha * 255.f;
} // ClearColor()
...
{ // TransformVertex()
	...
	// Convert to VCS
	glm::vec4 vertVCS = mvMatrixStack.top() * vert.position;
	...
	if (lightingEnabled and !phongEnabled) {
		...
		// Diffuse Light
		glm::vec3 norm = mvMatrixStack.top() * glm::vec4(vert.normal, 1.0f);
		glm::vec3 lightDir(lightPosition);
		float diffuse = std::max(glm::dot(glm::normalize(norm), glm::normalize(lightDir)), 0.f);
		...
		// Specular Light
		glm::vec3 eyeDir(vertVCS);
		glm::vec3 bisecDir = (lightDir + eyeDir) / 2.f;

		float specular = 0;

		// If the light is going to hit the surface directly
		if (glm::dot(norm, lightDir) > 0) {
			float ndotvb = glm::dot(norm, bisecDir) / (norm.length() * bisecDir.length());
		...
		glm::vec4 newVCol;
		newVCol.r = clamp(totalLight[0] * 255., 0.f, 255.f);
		newVCol.g = clamp(totalLight[1] * 255., 0.f, 255.f);
		newVCol.b = clamp(totalLight[2] * 255., 0.f, 255.f);
		newVCol.a = clamp(totalLight[3] * 255., 0.f, 255.f);
		vert.colour = newVCol;
	}

	// Convert to CCS
	glm::vec4 vertCCS = pMatrixStack.top() * vertVCS;

	// Convert to NDCS
	glm::vec3 vertNDCS(vertCCS.x / vertCCS.w, vertCCS.y / vertCCS.w, vertCCS.z / vertCCS.w);
	...
	sVert.position = glm::vec3(vertDCS.x, vertDCS.y, vertDCS.z);
...
{ // RasteriseLineSegment()
	glm::vec3 vector01 = vertex1.position - vertex0.position;
	glm::vec3 normal01(-vector01.y, vector01.x, 0.0);
	// normalise
	normal01 = glm::normalize(normal01);
...
// clipping will happen in the raster loop proper
	...
	glm::vec3 vector01 = vertex1.position - vertex0.position;
	glm::vec3 vector12 = vertex2.position - vertex1.position;
	glm::vec3 vector20 = vertex0.position - vertex2.position;

	// now compute the line normal vectors
	glm::vec3 normal01(-vector01.y, vector01.x, 0.0);
	glm::vec3 normal12(-vector12.y, vector12.x, 0.0);
	glm::vec3 normal20(-vector20.y, vector20.x, 0.0);

	// we don't need to normalise them, because the square roots will cancel out in the barycentric coordinates
	float lineConstant01 = glm::dot(normal01, vertex0.position);
	float lineConstant12 = glm::dot(normal12, vertex1.position);
	float lineConstant20 = glm::dot(normal20, vertex2.position);

	// and compute the distance of each vertex from the opposing side
	float distance0 = glm::dot(normal12, vertex0.position) - lineConstant12;
	float distance1 = glm::dot(normal20, vertex1.position) - lineConstant20;
	float distance2 = glm::dot(normal01, vertex2.position) - lineConstant01;
	...
	// this is here so that clipping works correctly
		...
		// this is also for correct clipping
			...
			glm::vec3 pixel(rasterFragment.col, rasterFragment.row, 0.);

			// right - we have a pixel inside the frame buffer AND the bounding box
			// note we *COULD* compute gamma = 1.0 - alpha - beta instead
			float alpha = (glm::dot(normal12, pixel) - lineConstant12) / distance0;
			float beta = (glm::dot(normal20, pixel) - lineConstant20) / distance1;
			float gamma = (glm::dot(normal01, pixel) - lineConstant01) / distance2;
			...
			if (lightingEnabled and phongEnabled) {
				...
				glm::vec3 fragNormal = alpha * vertex0.normal + beta * vertex1.normal + gamma * vertex2.normal;
				float fragAmb[4];
				float fragDiff[4];
				float fragSpec[4];
				float fragEmiss[4];
				...
				glm::vec3 fragEPos = alpha * glm::vec3(vertex0.ePos) + beta * glm::vec3(vertex1.ePos) + gamma * glm::vec3(vertex2.ePos);
				...
				// Diffuse Light
				glm::vec3 norm = mvMatrixStack.top() * glm::vec4(fragNormal, 1.0f);
				glm::vec3 lightDir(lightPosition);
				float diffuse = std::max(glm::dot(glm::normalize(norm), glm::normalize(lightDir)), 0.f);
			...
			if (textureEnabled) {
				...
				glm::vec3 fragTexCoord = alpha * vertex0.texCoord + beta * vertex1.texCoord + gamma * vertex2.texCoord;
				...
				if (texIndexIx < texture->width and texIndexIx >= 0 and texIndexIy < texture->height and texIndexIy >= 0) {
					if (texMode == FAKEGL_REPLACE) {
						...
						glm::vec4 texCol = texture->block[texIndexIy * texture->width + texIndexIx];
						float modifier[4] = { texCol.r, texCol.g, texCol.b, texCol.a };
						for (size_t i = 0; i < 4; i++) modifier[i] /= 255;

						// Multiply the fragment color by the texel color modifier
						rasterFragment.colour.r = rasterFragment.colour.r * modifier[0];
						rasterFragment.colour.g = rasterFragment.colour.g * modifier[1];
						rasterFragment.colour.b = rasterFragment.colour.b * modifier[2];
						rasterFragment.colour.a = rasterFragment.colour.a * modifier[3];
					}
				}
			}
...
{ // ProcessFragment()
	...
	if (frag.row >= 0 and frag.col >= 0 and frag.row < frameBuffer.height and frag.col < frameBuffer.width) {
		...
		if (dBufferingEnabled) {
			...
			if (cFragDepth > depthBuffer.block[fragIndex].a)
				// Stop processing the fragment
				return;
			else
				depthBuffer.block[fragIndex].a = cFragDepth;
		}

		// Draw the fragment to screen
		frameBuffer.block[fragIndex].r = frag.colour.r;
		frameBuffer.block[fragIndex].g = frag.colour.g;
		frameBuffer.block[fragIndex].b = frag.colour.b;
		frameBuffer.block[fragIndex].a = frag.colour.a;
	}
} // ProcessFragment()
...
{ // operator <<
	...
	outStream << "Draw Color: " << fakeGL.drawColour << std::endl;
