...
// if our old block is non-null
	...
	block = (glm::vec4*)calloc(Height * Width, sizeof(glm::vec4));
	if (block == NULL)
		return false;
...
glm::vec4* RGBAImage::operator [](const int rowIndex)
{ // row [] index operator
...
const glm::vec4* RGBAImage::operator [](const int rowIndex) const
{ // row [] index operator
// use pointer arithmetic to compute the row beginning
	return block + (rowIndex * width);
} // [] row index operator
